...
// Prototype which represents a sub chat action
function SubChatAction(inContext, inSettings) {
	// Init SubChatAction
	var instance = this;
	...
	function send(account, target) {
		var data = {};
		data.message = (target.state ? "/subscribers" : "/subscribersoff");
		ircSockets[account.id].sendMsg(data);
	}

	// Add event listeners
	document.addEventListener('/subscribers', function (evt) {
		instance.showSuccess.call(instance, evt.detail.account, evt.detail.value);
	});
	document.addEventListener('subs-only', function (evt) {
		instance.updateState.call(instance, evt.detail.account, evt.detail.value, 'subs-only');
	});
};
