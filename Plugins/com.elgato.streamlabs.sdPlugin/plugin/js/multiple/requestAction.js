...
// Prototype which represents a disable request action
function RequestAction(inContext, inSettings) {
	// Init RequestAction
	var instance = this;
	...
	// Private function called for the disable request request
	function disable(account, target, callback) {
		if (target.state) {
			var url = "https://streamlabs.com/api/v1.0/media-share/enable-requests";
		}
		else {
			var url = "https://streamlabs.com/api/v1.0/media-share/disable-requests";
		}
	...
	// Add event listeners
	document.addEventListener('mediaRequests', function (evt) {
		instance.updateState.call(instance, evt.detail.account, evt.detail.value, 'mediaRequests');
	});
};
