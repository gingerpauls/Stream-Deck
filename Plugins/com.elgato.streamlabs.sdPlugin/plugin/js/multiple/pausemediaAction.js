...
// Prototype which represents a pause media action
function PauseMediaAction(inContext, inSettings) {
	// Init PauseMediaAction
	var instance = this;
	...
	// Private function called for the pause media request
	function pause(account, target, callback) {
		if (target.state) {
			var url = "https://streamlabs.com/api/v1.0/media-share/play-media";
		}
		else {
			var url = "https://streamlabs.com/api/v1.0/media-share/pause-media";
		}
	...
	// Add event listeners
	document.addEventListener('mediaPlaying', function (evt) {
		instance.updateState.call(instance, evt.detail.account, evt.detail.value, 'mediaPlaying');
	});
};
