...
// Prototype which represents a disable backup list action
function BackupListAction(inContext, inSettings) {
	// Init BackupListAction
	var instance = this;
	...
	// Private function called for the disable backup list request
	function disable(account, target, callback) {
		if (target.state) {
			var url = "https://streamlabs.com/api/v1.0/media-share/enable-backup-playlist";
		}
		else {
			var url = "https://streamlabs.com/api/v1.0/media-share/disable-backup-playlist";
		}
	...
	// Add event listeners
	document.addEventListener('mediaBackupList', function (evt) {
		instance.updateState.call(instance, evt.detail.account, evt.detail.value, 'mediaBackupList');
	});
};
