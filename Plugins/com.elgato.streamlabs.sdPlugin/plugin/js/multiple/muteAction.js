...
// Prototype which represents a mute action
function MuteAction(inContext, inSettings) {
	// Init MuteAction
	var instance = this;
	...
	this.onKeyUp = function(inData, inCallback) {
		...
		inData.key = 'alertVolume';

		// Call actions onKeyUp method
		actionOnKeyUp.call(this, inData, mute);
		inCallback();
	};

	// Private function called for the mute alerts request
	function mute(account, target, callback) {
		if (target.state) {
			var url = "https://streamlabs.com/api/v1.0/alerts/unmute_volume";
		}
		else {
			var url = "https://streamlabs.com/api/v1.0/alerts/mute_volume";
		}
	...
	// Add event listeners
	document.addEventListener('alertVolume', function (evt) {
		instance.updateState.call(instance, evt.detail.account, evt.detail.value, 'alertVolume');
	});
};
