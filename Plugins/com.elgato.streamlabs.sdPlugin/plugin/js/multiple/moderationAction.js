...
// Prototype which represents a disable moderation action
function ModerationAction(inContext, inSettings) {
	// Init ModerationAction
	var instance = this;
	...
	// Private function called for the disable moderation request
	function disable(account, target, callback) {
		if (target.state) {
			var url = "https://streamlabs.com/api/v1.0/media-share/enable-moderation";
		}
		else {
			var url = "https://streamlabs.com/api/v1.0/media-share/disable-moderation";
		}
	...
	// Add event listeners
	document.addEventListener('mediaModeration', function (evt) {
		instance.updateState.call(instance, evt.detail.account, evt.detail.value, 'mediaModeration');
	});
};
