...
class OBS extends EventTarget {
	...
	try_connect() {
		...
		this._tryconnect = window.setTimeout(() => {
			this.connect();
		}, config.obs_connection_attempt_delay);
	}
	...
	connecting() {
		return (this._connecting) || ((this._ws) && (this._ws.readyState == 0));
	}
	...
	connected() {
		return (!this._connecting) && (this._ws && (this._ws.readyState === 1));
	}
	...
	send_version() {
		...
		this.call("version", () => { }, { "version": this._ctr && this._ctr._applicationInfo && this._ctr._applicationInfo.plugin && this._ctr._applicationInfo.plugin.version });
	}
