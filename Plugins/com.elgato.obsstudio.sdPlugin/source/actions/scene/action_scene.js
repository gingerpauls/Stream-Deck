...
class SceneAction extends IAction {
	constructor(controller) {
		...
		{ // Listeners
			...
			// Studio Mode
			this._api_studiomode.addEventListener("ready", (event) => {
				this._on_sm_ready(event); 
			});
			this._api_studiomode.addEventListener("enabled", (event) => {
				this._on_sm_enabled(event); 
			});
			this._api_studiomode.addEventListener("disabled", (event) => {
				this._on_sm_disabled(event); 
			});
	...
	_default_settings(settings) {
		...
		if ((settings.scene === undefined) || (typeof (settings.scene) !== "string")) {
			settings.scene = this._api_scene.preview;
		}
