...
class SceneItemVisibilityInspector extends IInspector {
	constructor(uPort, uUUID, uEvent, uInfo, uActionInfo) {
		...
		this._els = {
			...
			collections: document.querySelector("#collection .sdpi-item-value summary"),
		...
		this._have_collections = false;
		this._have_scenes = false;
		this._have_items = false;
		...
		this.addEventListener("rpc.scenes", (ev) => {
			this._on_rpc_scenes(ev); 
		});
		this.addEventListener("rpc.items", (ev) => {
	...
	validate() {
		if (!this._have_obs) {
			this.show_processing_overlay("Waiting for OBS...");
		} else if (!this._have_settings) {
			this.show_processing_overlay("Waiting for settings...");
		} else if (!this._have_scenes) {
			this.show_processing_overlay("Waiting for scenes...");
		} else if (!this._have_items) {
			this.show_processing_overlay("Waiting for sources...");
		} else {
			this.hide_overlays();
		}
	}
