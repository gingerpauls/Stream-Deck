...
class obs_scene_api extends EventTarget {
	...
	_on_obs_source_event_rename(ev) {
		...
		// Rename any scenes matching the event itself.
		if (this._scenes.has(prms["from"])) {
			this._scenes.set(prms["to"], this._scenes.get(prms["from"]));
			this._scenes.delete(prms["from"]);
		...
		this._enumeration = new Promise((resolve, reject) => {
			this.obs.call("obs.scene.items", (result, error) => {
				...
				if (error !== undefined) {
					...
					let items = new Map();
					for (let state of result) {
						// Try and find an existing item that matches the description to re-use resources.
						let item = this._items.get(state.id);
						if (item !== undefined) {
							if (!item.valid || (item.name !== state.name) || (item.group !== state.group)) {
								item.update(state);
							} else {
								item = undefined;
							}
						...
						// If there is none, create a new item.
						if (!item) {
							item = new obs_scene_item(this.obs, this, state);
						}
